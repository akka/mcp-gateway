...
@Component(id = "gmail-connection")
public class GmailConnectionEntity extends KeyValueEntity<GmailConnection> {
    ...
    public GmailConnectionEntity(Config config) {
        this.clientSecret = config.getString("gmail.oauth.client-secret");
    }

    @Override
    public GmailConnection emptyState() { return GmailConnection.empty(); }

    public ReadOnlyEffect<GmailConnection> getStatus() {
    ...
    public Effect<String> getAccessToken() {
        ...
        if (!state.isConnected()) {
            return effects().error("Gmail is not connected. Please complete the OAuth setup.");
        }
        if (!state.isTokenExpired()) {
            return effects().reply(state.accessToken());
        }
        if (state.refreshToken() == null) {
            return effects().error("Gmail token has expired. Please reconnect.");
        }
        try {
            ...
            return effects().error("Failed to refresh Gmail token: " + e.getMessage());
        }
    }
    ...
    private FreshToken performRefresh(GmailConnection connection) throws Exception {
        ...
        // Google only returns a new refresh token if rotation is enabled; keep old one otherwise

...
import io.akka.mcp.gateway.domain.ZohoConnection;
...
@Component(id = "zoho-connection")
public class ZohoConnectionEntity extends KeyValueEntity<ZohoConnection> {

    private static final HttpClient HTTP_CLIENT = HttpClient.newHttpClient();
    private static final ObjectMapper MAPPER = new ObjectMapper();

    @Override
    public ZohoConnection emptyState() {
        return ZohoConnection.empty();
    }

    public ReadOnlyEffect<ZohoConnection> getStatus() {
        return effects().reply(currentState());
    }

    /**
     * Returns a valid access token, refreshing transparently if the current one is expired.
     * Callers never need to check expiry or handle refresh themselves.
     */
    public Effect<String> getAccessToken() {
        var state = currentState();
        if (!state.isConnected()) {
            return effects().error("Zoho Desk is not connected. Please complete the OAuth setup.");
        }
        if (!state.isTokenExpired()) {
            return effects().reply(state.accessToken());
        }
        if (state.refreshToken() == null || state.tokenEndpoint() == null) {
            return effects().error("Zoho token has expired. Please reconnect.");
        }
        try {
            ...
            return effects().error("Failed to refresh Zoho token: " + e.getMessage());
        }
    }
    ...
    private FreshToken performRefresh(ZohoConnection connection) throws Exception {

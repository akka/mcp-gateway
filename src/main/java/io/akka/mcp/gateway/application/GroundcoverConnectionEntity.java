...
@Component(id = "groundcover-connection")
public class GroundcoverConnectionEntity extends KeyValueEntity<GroundcoverConnection> {

    public static final String ENTITY_ID = "groundcover";

    private static final HttpClient HTTP_CLIENT = HttpClient.newHttpClient();
    private static final ObjectMapper MAPPER = new ObjectMapper();

    @Override
    public GroundcoverConnection emptyState() {
        return GroundcoverConnection.empty();
    }

    public ReadOnlyEffect<GroundcoverConnection> getStatus() {
    ...
    public Effect<String> getAccessToken() {
        ...
        if (!state.isConnected()) {
            return effects().error("Groundcover is not connected. Please complete the OAuth setup.");
        }
        if (!state.isTokenExpired()) {
            return effects().reply(state.accessToken());
        }
        if (state.refreshToken() == null || state.tokenEndpoint() == null) {
            return effects().error("Groundcover token has expired. Please reconnect.");
        }
        try {
            ...
            return effects().error("Failed to refresh Groundcover token: " + e.getMessage());
        }
    }
    ...
    private FreshToken performRefresh(GroundcoverConnection connection) throws Exception {

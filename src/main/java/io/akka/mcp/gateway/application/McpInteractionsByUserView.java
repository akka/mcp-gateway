...
@Component(id = "mcp-interactions-by-user")
public class McpInteractionsByUserView extends View {
    ...
    public record UserPageRequest(String userId, int offset, int pageSize) {}

    public record PageRequest(int offset, int pageSize) {}

    @Consume.FromEventSourcedEntity(McpInteractionEntity.class)
    public static class McpInteractionsByUserUpdater extends TableUpdater<McpInteractionEntry> {
    ...
    @Query("SELECT * AS interactions, total_count() AS totalCount FROM mcp_interactions_by_user ORDER BY timestamp DESC OFFSET :offset LIMIT :pageSize")
    public QueryEffect<McpInteractionEntries> getAll(PageRequest request) {
        return queryResult();
    }

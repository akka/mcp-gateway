...
import io.akka.mcp.gateway.domain.GoogleDriveConnection;
...
@Component(id = "google-drive-connection")
public class GoogleDriveConnectionEntity extends KeyValueEntity<GoogleDriveConnection> {
    ...
    public GoogleDriveConnectionEntity(Config config) {
        this.clientSecret = config.getString("google-drive.client-secret");
    }

    @Override
    public GoogleDriveConnection emptyState() {
        return GoogleDriveConnection.empty();
    }

    public ReadOnlyEffect<GoogleDriveConnection> getStatus() {
    ...
    public Effect<String> getAccessToken() {
        ...
        if (!state.isConnected()) {
            return effects().error("Google Drive is not connected. Please complete the OAuth setup.");
        }
        if (!state.isTokenExpired()) {
            return effects().reply(state.accessToken());
        }
        if (state.refreshToken() == null || state.tokenEndpoint() == null) {
            return effects().error("Google Drive token has expired. Please reconnect.");
        }
        try {
            ...
            return effects().error("Failed to refresh Google Drive token: " + e.getMessage());
        }
    }
    ...
    private FreshToken performRefresh(GoogleDriveConnection connection) throws Exception {
